...
| `msoa21hclnm` | `text` | House of Commons Library readable MSOA name, joined at load on `msoa_code` (a 2021 code) from House of Commons Library MSOA Names v2.3 (13 February 2026). Populated only for open establishments; closed and legacy records, whose `msoa_code` is an older 2011 boundary, are left NULL. Open Parliament Licence. |
