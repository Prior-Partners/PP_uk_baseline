...
<img src="../../maps/edu_dfe_secondary_school_may2026.png" alt="Styling preview of edu_dfe_secondary_school_may2026" loading="lazy" style="width:100%;border:1px solid #d9d3c4;border-radius:8px;margin:6px 0 4px;">

<a href="http://localhost:7800/?layer=uk_baseline.edu_dfe_secondary_school_may2026" target="_blank" rel="noopener">Open in the Dashboard &#8599;</a> <span style="opacity:.6;font-size:.85em;">(start your local Dashboard first)</span>
...
- England only. 3,419 rows, one per secondary-phase open URN.
- Filter (re-stated for clarity): phase_of_education_name IN ('Secondary', 'Middle deemed secondary', 'All-through') AND establishment_status_name IN ('Open', 'Open, but proposed to close').
- Excludes Closed secondaries (~3.2k of the 6.7k Secondary phase records held by the parent are Closed and dropped here).
...
- This subset is a materialised filter, not a view. If the parent is refreshed, this table must be re-created.
- Same date-as-text caveat as the parent.
