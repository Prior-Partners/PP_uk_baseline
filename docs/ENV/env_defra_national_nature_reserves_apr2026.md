...
- Coastal note: MSOA coverage stops at the coastline (roughly Mean High Water), so split pieces retain 79.49% of the pre-split area of features that overlap an MSOA; 1 wholly offshore or non-England feature is kept whole with NULL geography columns. The full pre-split extent is uk.env_defra_national_nature_reserves_apr2026__preswap_jul03.
