...
- The row count counts split pieces, not source features: this layer was split by Middle Layer Super Output Area, so it holds one row per feature per MSOA piece, plus whole and remainder rows to keep 100% of the source geometry. 3,516 rows represent 1,925 source features, measured 25 August 2026. `source_fid` identifies the originating feature.
- Geography keys are NULL on 112 of 3,516 rows, measured 25 August 2026: 102 fall inside an England or Wales district and could carry one, so their keys are a gap rather than an absence; 10 are residual fragments left by the MSOA split, each under 100 sqm or 10 m.
...
| `ref_code` | `character varying` | Source field `ref_code`; Natural England LNR reference code (e.g. "1481551"). |
| `name` | `character varying` | Source field `name`; Local Nature Reserve name (e.g. "WOODGATE VALLEY"). |
| `measure` | `double precision` | Source field `measure`; reserve area as published. Unit: hectares. |
| `label` | `character varying` | Source field `label`; display label (e.g. "Woodgate Valley (LNR)"). |
...
| `geom` | `geometry(MultiPolygon,27700)` | Local Nature Reserve polygon geometry in EPSG:27700 (British National Grid); one part per MSOA (2021) after the split. |
