...
<p class="layer-short">World Heritage Sites</p>

`her_historicengland_world_heritage_sites_may2026`

<img src="../../maps/her_historicengland_world_heritage_sites_may2026.png" alt="Styling preview of her_historicengland_world_heritage_sites_may2026" loading="lazy" style="width:100%;border:1px solid #d9d3c4;border-radius:8px;margin:6px 0 4px;">
...
- The row count counts split pieces, not source features: this layer was split by Middle Layer Super Output Area, so it holds one row per feature per MSOA piece, plus whole and remainder rows to keep 100% of the source geometry. 905 rows represent 294 source features, measured 25 August 2026. `source_fid` identifies the originating feature.
- Geography keys are NULL on 31 of 905 rows, measured 25 August 2026: 10 fall inside an England or Wales district and could carry one, so their keys are a gap rather than an absence; 15 are residual fragments left by the MSOA split, each under 100 sqm or 10 m; 1 are in Scotland or Northern Ireland, outside the England and Wales lookup; 5 fall outside every district — offshore, inter-tidal, or beyond Great Britain.
...
|---|---|---|
| `source_fid` | `bigint` | Primary key of the source feature in the pre-split layer uk.her_historicengland_world_heritage_sites_may2026__preswap_jul03 (non-unique here: a feature spanning N MSOAs has N rows). |
| `fid_original` | `integer` |  |
| `listentry` | `integer` |  |
| `name` | `character varying` |  |
| `inscrdate` | `timestamp with time zone` |  |
